...
class Subregion:
    ...
    region_start: float
    region_end: float
    def __init__(self, region_start, region_end, cell_size,direction):
...
class Grid:
    def __init__(
        ...
        # 2) 初次将物理长度转换为整数网格点数（均匀网格逻辑，不传 axis）
        grid_xlength = self._handle_unit(
            [grid_xlength],
            grid_spacing=grid_spacing_x,
            axis=None
        )[0]
        grid_ylength = self._handle_unit(
            [grid_ylength],
            grid_spacing=grid_spacing_y,
            axis=None
        )[0]
        grid_zlength = self._handle_unit(
            [grid_zlength],
            grid_spacing=grid_spacing_z,
            axis=None
        )[0]
        ...
        # 4) 初始化节点坐标和单元尺寸（均匀网格）
        # 节点坐标数组长度为单元数+1
        self.x_coordinates = np.linspace(0, grid_xlength * grid_spacing_x, int(grid_xlength) + 1)
        self.y_coordinates = np.linspace(0, grid_ylength * grid_spacing_y, int(grid_ylength) + 1)
        self.z_coordinates = np.linspace(0, grid_zlength * grid_spacing_z, int(grid_zlength) + 1)
        # 计算单元尺寸（相邻节点间距）
        self.dx = np.diff(self.x_coordinates)
        self.dy = np.diff(self.y_coordinates)
        self.dz = np.diff(self.z_coordinates)

        # 5) 插入子区域（如果有）
        if subregions is not None:
            self.subregions = subregions
            for sub in subregions:
                self.add_subregion(sub)
        ...
        grid = fdtd.Grid(
            ...
            z_coordinates=self.z_coordinates,
        ...
        # 7) 保存到底层对象，并记录三向的长度和 background_index
        self._grid = grid
        self._grid_xlength = len(self.x_coordinates)-1
        self._grid_ylength = len(self.y_coordinates)-1
        self._grid_zlength = len(self.z_coordinates)-1
        self.background_index = np.sqrt(permittivity * permeability)
        self.flag_PML_not_set = True if set_PML else False

    def _handle_unit(self, lengths, grid_spacing=None, axis=None):
        """
        把“物理长度（米）”或“整数网格点” 转成 “整数索引”。
        lengths: List[int 或 float]，如果是 int，直接当作格点数；
                 如果是 float，就表示“物理长度”，需要映射到非均匀网格索引。
        axis: 'x','y','z’，若 axis=None，则走“均匀网格”分支。
        """
        converted = []
        for length in lengths:
            if isinstance(length, (int, np.integer)):
                converted.append(int(length))
            else:
                if length is None:
                    converted.append(None)
                else:
                    if axis is None:
                        idx = int(np.round(length / grid_spacing))
                        converted.append(idx)
                    else:
                        if axis == "x":
                            coords = self.x_coordinates  # 直接使用节点坐标数组
                        elif axis == "y":
                            coords = self.y_coordinates
                        elif axis == "z":
                            coords = self.z_coordinates
                        else:
                            raise ValueError("axis 必须是 'x','y' 或 'z'")

                        # 使用 side="right" - 1 获得最右边界索引
                        idx = np.searchsorted(coords, length, side="right") - 1
                        idx = max(min(idx, len(coords) - 2), 0)  # 确保索引在有效范围内
    ...
    def _calculate_time(self):
        # 对非均匀网格，使用物理长度计算
        n = np.sqrt(1.0 / self._grid.inverse_permittivity.min())
        Lx = np.sum(self.dx)
        Ly = np.sum(self.dy)
        Lz = np.sum(self.dz)
        L = max(Lx, Ly, Lz)
        time = int(L * 1.5 * n / constants.c / self._grid.time_step)
        return time
    ...
    def add_subregion(self, subregion):
        """
        在 X/Y/Z 方向插入一个子区域（非均匀细网格）
        subregion.direction:  "x"、"y" 或 "z"
        subregion.region_start, region_end: 物理坐标（米）
        subregion.cell_size: 细网格单元大小（米）
        """
        direction = subregion.direction
        ...
        if direction == "x":
            ...
            raise ValueError("add_subregion: direction 必须是 'x','y' 或 'z'")

        # 1. 找到 region_start 和 region_end 在节点坐标中的位置（索引）
        # 注意：节点坐标是递增的
        # 找到第一个大于等于region_start的节点索引
        start_idx = np.searchsorted(coords, region_start, side='left')
        # 找到第一个大于region_end的节点索引
        end_idx = np.searchsorted(coords, region_end, side='right')

        # 2. 检查是否在边界内
        if start_idx == 0 or end_idx > len(coords) - 1:
            raise ValueError("Subregion must be inside the grid and not at the very boundary.")

        # 3. 计算原网格中要替换的区间（从start_idx-1到end_idx）
        # 因为节点坐标是边界点，单元在节点之间，所以我们要替换的是从start_idx-1到end_idx的单元
        # 但我们不能直接替换节点，而是要在region_start和region_end处插入新的节点

        # 4. 生成细网格段的节点坐标
        # 细网格段的总长度
        total_length = region_end - region_start
        # 细网格单元数量
        num_cells = max(1, int(np.round(total_length / cell_size)))
        # 调整cell_size使得总长度不变
        cell_size_adj = total_length / num_cells
        # 生成细网格段的节点坐标（包括起始和结束点）
        new_coords_segment = np.linspace(region_start, region_end, num_cells + 1)

        # 5. 构建新的节点坐标数组
        # 原节点坐标在region_start之前的保留
        coords_before = coords[:start_idx]  # 全部 < region_start
        coords_after = coords[end_idx:]  # 全部 ≥ region_end
        # 合并
        new_coords = np.concatenate([coords_before, new_coords_segment, coords_after])

        # 6. 更新对应方向的节点坐标
        if direction == "x":
            self.x_coordinates = new_coords
            # 重新计算单元尺寸
            self.dx = np.diff(self.x_coordinates)
        elif direction == "y":
            self.y_coordinates = new_coords
            self.dy = np.diff(self.y_coordinates)
        elif direction == "z":
            self.z_coordinates = new_coords
            self.dz = np.diff(self.z_coordinates)



        # 没有返回值，只是更新了 self.x_coordinates / self.y_coordinates / self.z_coordinates
    ...
    #
    #     pass
    def add_object(self, obj: Waveguide):
        """
        将一个 Waveguide 或其它自定义对象插入到网格里，支持非均匀网格。
        这里的 obj._internal_objects 列表中，每个 internal_object 都
        包含物理坐标 (x, y, z) 以及物理尺寸 (xlength, ylength, zlength)。
        我们要把它们都映射为“非均匀网格索引区间”。
        """
        for internal_object in obj._internal_objects:
            # internal_object.x, .y, .z 以及 xlength 等，都是物理单位（米）或整数网格数
            if internal_object == 0:
                continue

            # ———————————— 1. 把物理坐标/尺寸映射到“非均匀网格索引” ————————————
            # 调用 _handle_unit，传入 axis="x"/"y"/"z" 来做 cumsum+searchsorted
            x_start = self._handle_unit([internal_object.x], axis="x", grid_spacing=None)[0]
            x_end = self._handle_unit([internal_object.x + internal_object.xlength], axis="x", grid_spacing=None)[0]
            y_start = self._handle_unit([internal_object.y], axis="y", grid_spacing=None)[0]
            y_end = self._handle_unit([internal_object.y + internal_object.ylength], axis="y", grid_spacing=None)[0]
            z_start = self._handle_unit([internal_object.z], axis="z", grid_spacing=None)[0]
            z_end = self._handle_unit([internal_object.z + internal_object.zlength], axis="z", grid_spacing=None)[0]

            # ———————————— 2. 检查索引范围是否越界 ————————————
            self._check_parameters(x_start, x_end, y_start, y_end, z_start, z_end, object_to_check=None,
                                   name=internal_object.name)

            # ———————————— 3. 在底层网格 _grid 上填充 Object ————————————
            self._grid[
            x_start: x_end,
            y_start: y_end,
            z_start: z_end
            ] = fdtd.Object(
                permittivity=internal_object.permittivity,
                name=internal_object.name,
    ...
    def set_detector(
        ...
        @param x_start, y_start, z_start, x_end, y_end, z_end: for 'linedetector'
        @param x, y, z: center position for 'blockdetector'
        @param xlength, ylength, zlength: size for 'blockdetector'
        @param name:
        @param axis: "x", "y", or "z" for blockdetector
        """

        # 默认放在中心
        if x is None:
            x = self._grid_xlength // 2
        if y is None:
            y = self._grid_ylength // 2
        if z is None:
            z = self._grid_zlength // 2

        # —— 替换为 axis 模式 ——
        # linedetector 需要四个端点，blockdetector /中心+长度也可以用 axis 模式
        xlength, x, x_start, x_end = self._handle_unit(
            [xlength, x, x_start, x_end], axis="x"
        )
        ylength, y, y_start, y_end = self._handle_unit(
            [ylength, y, y_start, y_end], axis="y"
        )
        zlength, z, z_start, z_end = self._handle_unit(
            [zlength, z, z_start, z_end], axis="z"
        )

        if detector_type == 'linedetector':
            # 如果没有给 start/end，就以 center±length/2 自动生成
            if x_start is None:
                x_start = x - xlength // 2
                x_end = x_start + xlength
            if y_start is None:
                y_start = y - ylength // 2
                y_end = y_start + ylength
            if z_start is None:
                z_start = z - zlength // 2
                z_end = z_start + zlength

            self._check_parameters(x_start, x_end, y_start, y_end, z_start, z_end, name=name)
            self._grid[x_start:x_end, y_start:y_end, z_start:z_end] = fdtd.LineDetector(name=name)

        elif detector_type == 'blockdetector':
            # 不指定 axis，则自动在单元数为1的方向绘制
            if axis is None:
                dims1 = [i for i, sz in enumerate(self._grid.inverse_permittivity.shape) if sz == 1]
                if not dims1:
                    raise ValueError("Parameter 'axis' 必须在 3D blockdetector 中指定")
                axis = conversions.number_to_letter(dims1[0])

            # 以 center±length/2 计算 start/end
            x0 = x - xlength // 2
            y0 = y - ylength // 2
            z0 = z - zlength // 2

            if axis == "x":
                self._check_parameters(x0, x0, y0, y0 + ylength, z0, z0 + zlength, name=name)
                self._grid[x0:x0, y0:y0 + ylength, z0:z0 + zlength] = fdtd.BlockDetector(name=name, axis=axis)
            elif axis == "y":
                self._check_parameters(x0, x0 + xlength, y0, y0, z0, z0 + zlength, name=name)
                self._grid[x0:x0 + xlength, y0:y0, z0:z0 + zlength] = fdtd.BlockDetector(name=name, axis=axis)
            elif axis == "z":
                self._check_parameters(x0, x0 + xlength, y0, y0 + ylength, z0, z0, name=name)
                self._grid[x0:x0 + xlength, y0:y0 + ylength, z0:z0] = fdtd.BlockDetector(name=name, axis=axis)
            else:
                raise ValueError("Invalid axis for blockdetector: " + str(axis))

        else:
            raise ValueError("Invalid detector type: " + str(detector_type))
    ...
    def plot_n(self,
        ...
        if axis == "x":
            # 剖面是在 x=const，横轴 y，纵轴 z
            xlabel, ylabel = 'y/μm', 'z/μm'
            coords1 = self.y_coordinates  # y 节点
            coords2 = self.z_coordinates  # z 节点
        elif axis == "y":
            xlabel, ylabel = 'x/μm', 'z/μm'
            coords1 = self.x_coordinates
            coords2 = self.z_coordinates
        else:  # axis == "z"
            xlabel, ylabel = 'x/μm', 'y/μm'
            coords1 = self.x_coordinates
            coords2 = self.y_coordinates
    ...
    def read_simulation(folder: str = None):
        ...
        if not folder.endswith(".npz"):
            folder_npz = folder + r"\saved_grid.npz"
        try:
            readings = np.load(folder_npz, allow_pickle=True)
        except:
            folder_npz = folder + r"\detector_readings.npz"
            readings = np.load(folder_npz, allow_pickle=True)
    ...
    def plot_field(grid=None, axis="y", axis_index=0, field="E", field_axis=None, folder=None, cmap="jet",
        ...
        plt.figure()
        if axis.lower() == "x":
            coords1 = grid.y_coordinates  # 横轴是 y
            coords2 = grid.z_coordinates  # 纵轴是 z
            xlabel, ylabel = 'y/μm', 'z/μm'
        elif axis.lower() == "y":
            coords1 = grid.x_coordinates
            coords2 = grid.z_coordinates
            xlabel, ylabel = 'x/μm', 'z/μm'
        else:  # axis == "z"
            coords1 = grid.x_coordinates
            coords2 = grid.y_coordinates
            xlabel, ylabel = 'x/μm', 'y/μm'
        ...
        if show_geometry:
            ...
            x_centers = (coords1[:-1] + coords1[1:]) * 1e6 / 2  # μm
            y_centers = (coords2[:-1] + coords2[1:]) * 1e6 / 2  # μm

            # 3. 生成网格：X, Y 维度都和 contour_data.T 匹配
    ...
    def visualize_single_detector(self,
        ...
        if data.ndim == 3:
            ...
            # 绘制 2D 颜色图
            im = axes[0][0].imshow(np.transpose(np.squeeze(data[time_step_max_field, :, :, :, field_axis])),
