...
class PML(Boundary):
    ...
    def update_phi_E(self, dx=None, dy=None, dz=None):
        ...
        """
        # dx, dy, dz 是调用 update_phi_E 时传入的局部间距数组
        # 此处 dx 是 shape=(Nx,), 要取 dx[1:] 和广播
        inv_dx = 1.0 / dx[1:]
        inv_dy = 1.0 / dy[1:]
        inv_dz = 1.0 / dz[1:]

        inv_dx_x = inv_dx[:, None, None]  # (Nx-1, 1, 1)
        inv_dy_y = inv_dy[None, :, None]  # (1, Ny-1, 1)
        inv_dz_z = inv_dz[None, None, :]  # (1, 1, Nz-1)

        # 更新 psi_Ey，其中 dx_local 必须是局部的、已裁剪的
        self.psi_Ex[:, 1:, :, 1] += (Hz[:, 1:, :] - Hz[:, :-1, :]) * c[:, 1:, :, 1] * inv_dy_y
        self.psi_Ex[:, :, 1:, 2] += (Hy[:, :, 1:] - Hy[:, :, :-1]) * c[:, :, 1:, 2] * inv_dz_z

        self.psi_Ey[:, :, 1:, 2] += (Hx[:, :, 1:] - Hx[:, :, :-1]) * c[:, :, 1:, 2] * inv_dz_z
        self.psi_Ey[1:, :, :, 0] += (Hz[1:, :, :] - Hz[:-1, :, :]) * c[1:, :, :, 0] * inv_dx_x

        self.psi_Ez[1:, :, :, 0] += (Hy[1:, :, :] - Hy[:-1, :, :]) * c[1:, :, :, 0] * inv_dx_x
        self.psi_Ez[:, 1:, :, 1] += (Hx[:, 1:, :] - Hx[:, :-1, :]) * c[:, 1:, :, 1] * inv_dy_y
    ...
    def update_phi_H(self, dx=None, dy=None, dz=None):
        ...
        """
        # 从 Grid 拿到局部间距数组（长度分别为 Nx, Ny, Nz）
        inv_dx = 1.0 / dx[:-1]  # 对应 Hx[:, :, :] 中 [1:, :, :]
        inv_dy = 1.0 / dy[:-1]
        inv_dz = 1.0 / dz[:-1]
        ...
        # === 更新 psi_Hx：∂Ez/∂y 和 ∂Ey/∂z ===
        self.psi_Hx[:, :-1, :, 1] += (Ez[:, 1:, :] - Ez[:, :-1, :]) \
                                     * c[:, :-1, :, 1] * inv_dy_y
        self.psi_Hx[:, :, :-1, 2] += (Ey[:, :, 1:] - Ey[:, :, :-1]) \
                                     * c[:, :, :-1, 2] * inv_dz_z

        # === 更新 psi_Hy：∂Ex/∂z 和 ∂Ez/∂x ===
        self.psi_Hy[:, :, :-1, 2] += (Ex[:, :, 1:] - Ex[:, :, :-1]) \
                                     * c[:, :, :-1, 2] * inv_dz_z
        self.psi_Hy[:-1, :, :, 0] += (Ez[1:, :, :] - Ez[:-1, :, :]) \
                                     * c[:-1, :, :, 0] * inv_dx_x

        # === 更新 psi_Hz：∂Ey/∂x 和 ∂Ex/∂y ===
        self.psi_Hz[:-1, :, :, 0] += (Ey[1:, :, :] - Ey[:-1, :, :]) \
                                     * c[:-1, :, :, 0] * inv_dx_x
        self.psi_Hz[:, :-1, :, 1] += (Ex[:, 1:, :] - Ex[:, :-1, :]) \
                                     * c[:, :-1, :, 1] * inv_dy_y
