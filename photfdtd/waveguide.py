...
class Waveguide:
    ...
    def __init__(
        ...
        # === Step 1: 获取仿真中心点的物理坐标（单位: 米）===
        if x is None:
            x = grid.x_coordinates[len(grid.x_coordinates) // 2]
        if y is None:
            y = grid.y_coordinates[len(grid.y_coordinates) // 2]
        if z is None:
            z = grid.z_coordinates[len(grid.z_coordinates) // 2]

        # === Step 2: 根据中心 + 物理长度 计算边界 ===
        def compute_index_and_length(center: float, length: float, axis: str):
            pmin = center - length / 2
            pmax = center + length / 2
            i0 = grid._handle_unit([pmin], axis=axis)[0]
            i1 = grid._handle_unit([pmax], axis=axis)[0] + 1  # 包含右边
            return i0, i1 - i0  # 起始索引，格点数

        self.x, self.xlength = compute_index_and_length(x, xlength, 'x')
        self.y, self.ylength = compute_index_and_length(y, ylength, 'y')
        self.z, self.zlength = compute_index_and_length(z, zlength, 'z')

        # === Step 3: 保存中心索引（不是物理坐标）===
        self.x_center = self.x + self.xlength // 2
        self.y_center = self.y + self.ylength // 2
        self.z_center = self.z + self.zlength // 2

        # === Step 4: reset_xyz 表示重新定位器件起点为中心偏移 ===
        if reset_xyz:
            self.x = self.x_center - self.xlength // 2
            self.y = self.y_center - self.ylength // 2
            self.z = self.z_center - self.zlength // 2
        else:
            self.x = copy(self.x)
            self.y = copy(self.y)
            self.z = copy(self.z)

        # === Step 5: 宽度处理（如为物理量，则映射为格点数）===
        if width is None:
            self.width = self.xlength
        else:
            self.width = grid._handle_unit([width], axis="x")[0]
